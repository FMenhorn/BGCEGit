...
The projection might lead to parameters $u,v\not\in\left[0,1\right]^2$ for some of the datapoints. As these are not directly above a \ac{quad}, they might lead to inconsistencies in where the datapoints of a quad belong to, especially along interfaces of quads, where points on different quads might lie overlapping\todourgent[author=Benni]{@Erik: Whaaaaaat?}. To simplify the treatment, these inconsistent points are omitted.
