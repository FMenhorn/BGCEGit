...
Therefore, in our algorithm we are reconstructing the surface on two different scales, a coarse and a fine scale\todourgent[author=Benni]{explain our coarsening strategy}. The coarse scale surface reconstruction will be used as a patch distribution - from the fine scale we obtain our datapoints to which the \ac{NURBS} will be fitted to. We call this approach \emph{Two-scale \acl{DC}} (see \autoref{fig:TwoScale}).
