...
\tododone[author=erik]{{@}Saumi: can you homogenize this and next section and decide if both should have numerated lists or bullet points?}
\tododone[author=saumi]{@Erik: enumerated lists seemed to suit algorithms better, so changed to that}
\begin{enumerate}
...
\item The row of $\lsControlPointCoefMatrix(\vec{u}\vec{v})\lsControlPointCoefMatrix^{PS}$ is saved in a sparse matrix\footnote{The sparse matrices for the surface reconstructed part are implemented using SciPy \cite{Scipy}}\tododone[author=erik]{input the right citation of scipy}. 
\end{enumerate}
