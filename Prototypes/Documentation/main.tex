...
\begin{document}
	...
	\item Pysparse:
		...
		\end{itemize}
	\end{itemize}
	Furthermore, since by default the output .vtk files from Topy are binary, we need to change them to ascii. Hence, in the Topy source file \texttt{core/visualization.py} in the method \texttt{{\_}write{\_}legace{\_}vtu(x, fname)} (line 160), change in line 194 \texttt{binary} to \texttt{ascii}. \todo{Code picture}
	...
	\texttt{python optimize.py <example.tpd-file>} \todo{Output picture}\\
	...
	\item cd oce
	\item mkdir build
	\item cmake .. \todo{Output picture}
	\item make \todo{Output picture}
	\item sudo make install \todo{Output picture} 
	\end{itemize}
	In the make step, one can use the \texttt{-jx} parameter where x is the number of processor to build in parallel. These steps are in accord with the installation guide on the git page itself. One can also use the cmake-gui \todo{Output picture} to change some of the build configuration if need be (e.g. include OpenMP support).
	\section{Installation Test}
	Test the installation by calling \\
	\texttt{make test} \todo{Output picture}\\
