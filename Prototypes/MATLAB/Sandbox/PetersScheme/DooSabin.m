...
for j=1:size(faces,1)
    ...
    vertices_refined(n*j - n + 1:n*j, 1) = (1 - alpha).*X + alpha.*F(1).*ones(1, length(X))';
    vertices_refined(n*j - n + 1:n*j, 2) = (1 - alpha).*Y + alpha.*F(2).*ones(1, length(X))';
    vertices_refined(n*j - n + 1:n*j, 3) = (1 - alpha).*Z + alpha.*F(3).*ones(1, length(X))';
    ...
    for k = 0:n_local-1
        ...
        verts_children(faces(j, k+1), ind) = n*j-n+k+1;
    ...
    for k = 0:n_local-1
        ...
        edges_children(current_edge_id, ind) = n*j-n + k + 1;
        edges_children(current_edge_id, ind+1) = n*j -n + mod(k+1,n) + 1;
        
    end
    
    %add the face formed by children vertices of the old face j
    faces_refined(j, 1:n_local) = n*j-n+1:n*j;
    colours(j) = 1;
end
%verts_children
...
for i = 1:N
    ...
    if (length(new_face) >= 3)
        ...
        for iter = 1:length(new_face)-1
            ...
            for j = 2:length(neighbouring_faces)
                ...
                if (length(intersect(faces(neighbouring_faces(k),:),faces(neighbouring_faces(j),:))) == 2) && (isempty(find(face_ordered == new_face(j))))
