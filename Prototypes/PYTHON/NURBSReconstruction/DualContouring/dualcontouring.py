...
def coarsen_dataset(coarsening_steps, fine_dataset):
    ...
    if coarsening_steps > 0:
        ...
        for x, y, z in it.product(np.arange(fine_dataset._dimensions['min'][0], fine_dataset._dimensions['max'][0],
                                            fine_dataset._resolution),
                                  np.arange(fine_dataset._dimensions['min'][1], fine_dataset._dimensions['max'][1],
                                            fine_dataset._resolution),
                                  np.arange(fine_dataset._dimensions['min'][2], fine_dataset._dimensions['max'][2],
                                            fine_dataset._resolution)):
...
def dual_contour(dataset, res_fine, is_coarse_level, do_manifold_treatment):
    ...
    for x, y, z in it.product(
            np.arange(dataset._dimensions['min'][0], dataset._dimensions['max'][0], dataset._resolution),
            np.arange(dataset._dimensions['min'][1], dataset._dimensions['max'][1], dataset._resolution),
            np.arange(dataset._dimensions['min'][2], dataset._dimensions['max'][2], dataset._resolution)):
        ...
        for i in range(3):
            for j in range(i):
                ...
                in vindex:
                    ...
                    key_ijk = tuple(o + dataset._resolution * dirs[i] + dataset._resolution * dirs[j] + dataset._resolution * dirs[k])
                    ...
                    if c != d:
                        dc_quads.append([vindex[tuple(o)], vindex[tuple(o + dataset._resolution * dirs[i])],
                                         vindex[tuple(o + dataset._resolution * dirs[i] + dataset._resolution * dirs[j])],
                                         vindex[tuple(o + dataset._resolution * dirs[j])]])

    if do_manifold_treatment:
        dc_verts, dc_quads, dc_manifold_edges = resolve_manifold_edges(dc_verts, vindex, dc_quads, dataset, dataset._resolution)
    else:
