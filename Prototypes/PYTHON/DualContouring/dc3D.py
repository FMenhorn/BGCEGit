...
def tworesolution_dual_contour(dataset, resolutions, dims):
    [dc_verts_fine, dc_quads_fine, dc_manifold_edges_fine, not_consistent_edges_fine] = dual_contour(dataset,
                                                                          resolutions['fine'],
                                                                          dims,
                                                                          coarse_level=False)
    [dc_verts_coarse, dc_quads_coarse, dc_manifold_edges_coarse, not_consistent_edges_coarse] = dual_contour(dataset,
                                                                                resolutions['coarse'],
                                                                                dims,
                                                                                coarse_level=True)
    ...
    dc_not_consistent_edges = {'fine': not_consistent_edges_fine, 'coarse': not_consistent_edges_coarse}

    return dc_verts, dc_quads, dc_manifold_edges, dc_not_consistent_edges
...
def dual_contour(data, res, dims, coarse_level):
    ...
    if coarse_level:
        dc_verts, dc_quads, manifold_edges,not_consistent_edges = resolve_manifold_edges(dc_verts, vindex, dc_quads, data, res)
    else:
        manifold_edges = create_manifold_edges(dc_quads, vindex, data, res)
        not_consistent_edges = []

    return np.array(dc_verts), dc_quads, manifold_edges, not_consistent_edges