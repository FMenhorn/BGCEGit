...
class ManifoldEdge:
    def __init__(self, _manifold_edge_key, _manifold_edge_quads, _manifold_edge_vertex_quads, _dc_vindex, _dataset, _resolution):
        ...
        self.middle_sign = _dataset[self.middle_value_key] > 0
        ...
        for i in range(3):
            for j in range(i):
                if tuple(o + res*dirs[i]) in vindex and tuple(o + res*dirs[j]) in vindex and tuple(o + res*(dirs[i] + dirs[j])) in vindex:
                    k = 3-(i+j) # normal id
                    if (data[tuple(o+res*dirs[i]+res*dirs[j])] > 0) \
                            != \
                            (data[tuple(o+res*dirs[i]+res*dirs[j]+res*dirs[k])] > 0): # dir[i]+dir[j]+dir[k] = [1,1,1] for all!
