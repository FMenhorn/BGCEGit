...
def gradient_from_data(data,res):
    gdata={}
    for key in data:
        key_xp1=tuple(np.array(key)+np.array([res,0.0]))
        key_xm1=tuple(np.array(key)-np.array([res,0.0]))
        key_yp1=tuple(np.array(key)+np.array([0.0,res]))
        key_ym1=tuple(np.array(key)-np.array([0.0,res]))
        if key_xm1 in data and key_xp1 in data and key_ym1 in data and key_yp1 in data:
            dx=(int(is_inside(data,key_xp1))-int(is_inside(data,key_xm1)))
            dy=(int(is_inside(data,key_yp1))-int(is_inside(data,key_ym1)))
            gdata[key] = [dx,dy]
        else:
            gdata[key] = [0.0,0.0]
