...
\tododone[inline]{Benni: explain our approach without hermite data (only root, no gradient, only boolean values) of taking the mean value of the roots, drawbacks of simplification. Do something like a proof why this is resonable.}
Computing the position of the new node by just taking the mean value of all the roots on the \acsp{SignChangingEdge} of one cube represent an easier approach, which does not rely on gradient information, but is also not able to represent sharp features. The resulting vertex is obviously\todourgent{obvious?} inside the cube, because it represents the average of nodes lying on the edges of the cube. 
