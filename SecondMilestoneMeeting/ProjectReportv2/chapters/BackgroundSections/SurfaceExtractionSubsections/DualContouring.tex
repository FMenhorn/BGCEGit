...
\end{center}
\end{subfigure}
\caption{Comparision of \ac{MC} and \ac{DC} for identical datasets. The vertices are created on the edges of the cubes for \ac{MC} (\autoref{sfig:MC}) and inside the cubes for \ac{DC} (\autoref{sfig:DC}). Please note that the sharp feature in the top right cube can only be reconstructed by \ac{DC}. Figure from \cite{}.}
\label{fig:CompMCDC}
\end{center}
\end{figure}
\todourgent[inline,author=Benni]{Reference to \cite{Hermite2002} in \autoref{fig:CompMCDC} was wrong! Fix this -> JC}
