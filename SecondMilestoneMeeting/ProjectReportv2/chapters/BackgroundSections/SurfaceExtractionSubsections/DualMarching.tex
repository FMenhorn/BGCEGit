...
\todourgent[inline]{Benni: this part is more a outlook than something which has been implemented, remove this and keep it for next milestone? If yes, don't forget to remove the references in the introduction of this chapter!}
...
\tododone[inline]{Benni: Change table | Erik: I added newline - separates out the different points. Also think maybe different bullets than in text bullet lists - minuses maybe?}
\begin{table}[H]
...
Even though the method is working on tetrahedra, we can still apply it to a voxel dataset, by composing each cube out of 5 or 6 tetrahedra (\autoref{fig:splittingCubes}). But this high amount of simplifictaion comes with a drawback: The treatment of ambiguous cases depends on the splitting scheme applied to a cube\todointern{proof or reference! Show 2D example.}. Further detail on this method can be found in \cite{Nielson2008}.
