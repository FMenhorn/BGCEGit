...
\end{itemize}
\subsubsection{From a fine to a coarse quad mesh \todourgent{better caption?--IDEA: Two scale Dual COntouring!!}}
...
\begin{itemize}
\item We cannot guarantee that the coarse and the fine scale have exactly the same topology. Topological details of the fine scale, which do not exist on the coarse scale, are lost.\todourgent{should we mention "small topological details" ? otherwise it looks like we are doing a very bad job}
