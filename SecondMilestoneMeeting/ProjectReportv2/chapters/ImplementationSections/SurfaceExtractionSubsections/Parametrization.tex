...
\end{itemize}
\subsubsection{Two-scale Dual Contouring \tododone{better caption?--IDEA: Two scale Dual COntouring!!}}
...
\begin{itemize}
\item We cannot guarantee that the coarse and the fine scale have exactly the same topology. Small topological details of the fine scale, which do not exist on the coarse scale, are lost.\tododone{should we mention "small topological details" ? otherwise it looks like we are doing a very bad job}
...
\end{itemize}
\todointern[inline]{refer to future work section for possible optimizations}
